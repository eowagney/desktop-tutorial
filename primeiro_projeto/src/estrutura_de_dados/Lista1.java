...
public class Lista1 {
    ...
    public static void inserir(Scanner scanner, ArrayList<Object[]> listaProdutos) {
        ...
        String novoNome = ((String) dados[1]).toLowerCase();
        int i = 0;
        while (i < listaProdutos.size()) {
            String nomeExistente = ((String) listaProdutos.get(i)[1]).toLowerCase();
            if (novoNome.compareTo(nomeExistente) < 0) {
                break;
            }
            i++;
    }
        listaProdutos.add(i, dados); 
    ...
    public static void remover(Scanner scanner, ArrayList<Object[]> listaProdutos) {
        ...
        for (int i = 0; i < listaProdutos.size(); i++) {
            if ((int) listaProdutos.get(i)[0] == codigo) {
